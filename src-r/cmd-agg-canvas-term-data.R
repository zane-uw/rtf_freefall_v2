...
# User input
#   term, e.g. au20
#   relpath to project directory ()
#   this system's dns entry for edw connection

optls <- list(make_option(c('--term'),
                          type = 'character',
                          default = NULL,
                          help = 'academic terms, e.g. au20, su04 [default = %default]'),
              make_option(c('--dir'),
                          type = 'character',
                          default = NULL,
...
if (is.null(opts$dir)){
  print_help(optprs)
  stop('Must supply relative path to project directory (--dir [...])', call. = FALSE)
}
if (is.null(opts$term)){
  print_help(optprs)
  stop('Must supply academic term argument (--term [...])', call. = FALSE)
}
